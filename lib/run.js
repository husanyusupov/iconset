...
import {filesInDirectory, getAttr, useCurrentColor} from "./utils.js";
...
function getSvgContent(file) {
  try {
    ...
    if (match) {
      content = useCurrentColor(match[2].trim());
      viewBox = getAttr(match[1], 'viewBox');
      fill = getAttr(match[1], 'fill');
    }
